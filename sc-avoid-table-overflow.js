...
postscriptum.plugin('sc-avoid-table-overflow', (processor, options) => {
    ...
    processor.on('valid-break-point', function () {
        if (this.layoutContext.parentCtx)
            return;
        const { breakedBox, breakPoint } = this.layoutContext;
        if (breakedBox && breakedBox.mainBlock) {
            const startNode = ranges.pointEquals(breakPoint, ranges.positionBefore(breakedBox.startBlock)) ? breakedBox.startBlock : breakedBox.mainBlock;
            const tables = [];
            let table = startNode;
