...
postscriptum.plugin('sc-avoid-table-widows', (processor) => {
    ...
    processor.on('valid-break-point', function () {
        ...
        if (flag == "tested" || flag == "reduced")
            return;
        const { breakPoint, breakedBox } = this.layoutContext;
        if (breakedBox.mainBlock && walk.isRow(getComputedStyle(breakedBox.mainBlock)) == 'table-row') {
            const breakedTable = walk.ancestor(breakedBox.mainBlock, isTable);
            if (!breakedTable || breakedTable.rows.length <= 1)
                return;
